...
def MDQUALITYTABLE(data,minduration,outPutFileIR):
    ...
    for run in data:
        ...
        polarity = 'plus'
        print(run['aliceL3Polarity'])
        if (run['aliceL3Polarity'] == 'POSITIVE'):
            polarity = 'plus'
        else:
            polarity = 'minus'

        outPutFileIR.write(str(run['runNumber']) + "\t" + str(run['inelasticInteractionRateAvg']) + "\t" + polarity + "\t" + str(run['timeTrgStart']) + "\t" + str(run['timeTrgEnd']) + "\n")
...
def EOR(data,SavePng):
    ...
    for run in data:
        ...
        try:
            ...
            eor = 'None'
        # if IsCosmics(run):
        if IsRunType(run, "PHYSICS"):
            if eor in physicsdict:
                try:
                    physicsdict[eor].append([run['runNumber'], run['eorReasons'][0]['title'], run['eorReasons'][0]['description']])
                except:
                    physicsdict[eor].append([run['runNumber'],'',''])
            else:
                try:
                    physicsdict[eor] = [[run['runNumber'], run['eorReasons'][0]['title'], run['eorReasons'][0]['description']],]
                except:
                    physicsdict[eor] = [[run['runNumber'],'',''],]
        elif IsRunType(run, "COSMICS"):
            if eor in cosmicsdict:
                try:
                    cosmicsdict[eor].append([run['runNumber'], run['eorReasons'][0]['title'], run['eorReasons'][0]['description']])
                except:
                    cosmicsdict[eor].append([run['runNumber'],'',''])
            else:
                try:
                    cosmicsdict[eor] = [[run['runNumber'], run['eorReasons'][0]['title'], run['eorReasons'][0]['description']],]
                except:
                    cosmicsdict[eor] = [[run['runNumber'],'',''],]
        else:
    ...
    print(' --- PHYSICS --- ')
    for eor in physicsdict:
        print(eor,':',len(physicsdict[eor]))
        if eor != 'Run Coordination':
            print('       ',physicsdict[eor])
    print(' --- COSMICS --- ')
    ...
    ListEOR = [eor for eor in DICT if ((eor != 'Run Coordination') and (eor != 'None'))]
    ListN = [len(DICT[eor]) for eor in DICT if ((eor != 'Run Coordination') and (eor != 'None'))]

    fig, ax = plt.subplots()
    palette = sns.color_palette('deep')
    # palette[ListEOR.index('Run Coordination')] = (0.8,0.8,0.8)
    # ax.pie(ListN, labels=ListEOR, colors=palette, startangle=80)
    ax.pie(ListN, labels=ListEOR, autopct='%1.1f%%', startangle=80)

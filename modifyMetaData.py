...
def main():
    ...
    for startrun,periodRun in zip(startTimes,periodRuns):
        ...
        outFile.write("curl -i -k --cert ~/.globus/usercert.pem --key ~/.globus/userkey.pem -H -L -X PUT \'http://alice-ccdb.cern.ch/MID/Calib/ChamberEfficiency/" + str(startrun) + "/" + etag + "?Created=1640991600000\'\n")
        """
        ...
        """
        print("curl -i -k --cert ~/.globus/usercert.pem --key ~/.globus/userkey.pem -H -L -X PUT \'http://alice-ccdb.cern.ch/MID/Calib/ChamberEfficiency/" + str(startrun) + "/" + etag + "?Created=1640991600000\'")    
